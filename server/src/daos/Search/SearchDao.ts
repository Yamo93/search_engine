...
class UserDao implements IUserDao {
  ...
  constructor(pageDb: PageDb, fileService: FileService) {
    this.pageDb = pageDb;
  ...
  private async load() {
    await this.fileService.readPages(this.pageDb);
  ...
  private query(query: string, searchMode: string): SearchResult[] {
    ...
    for (let i = 0; i < this.pageDb.pages.length; i++) {
      ...
      if (page?.pageRank) scores.pageRanks[i] = page?.pageRank;
    }

    // normalize scores
    this.normalize(scores.content, false);
    this.normalize(scores.location, true);
    this.normalize(scores.pageRanks, false);
  ...
  calculateScore(
    ...
    if (this.isAdvanced(searchMode)) {
      ...
      return (
        ...
        0.8 * scores.location[index] +
        0.5 * scores.pageRanks[index]
      );
    }
